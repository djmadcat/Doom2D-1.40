...
static void *sndswn;
static void *sndswx;
static void *sndnoway;
static void *sndbdo;
static void *sndbdc;
static void *sndnotele;
...
void Z_water_trap(obj_t *o) {
    ...
    if ((y = o->y) >= FLDH * CELH + o->h) {
        return;
    }
    if ((x = o->x) < 0 || o->x > FLDW * CELW) {
        return;
    }
    sx = (x - o->r) / CELW;
    sy = (y - o->h + 1) / CELH;
    ...
    for (i = p = 0; i < MAXSW; ++i) {
        if (sw[i].t && !sw[i].tm) {
            if (sw[i].x >= sx && sw[i].x <= x && sw[i].y >= sy && sw[i].y <= y && ((sw[i].f & 0x8F) & t)) {
                ...
                if (sw[i].tm) {
                    fldb[sw[i].y][sw[i].x] = walswp[fldb[sw[i].y][sw[i].x]];
                    p = 1;
                }
                if (sw[i].tm == 1) {
                    sw[i].tm = 0;
