...
unsigned char fx_scr1[V_SCR_WIDTH * V_SCR_HEIGHT];
unsigned char fx_scr2[V_SCR_WIDTH * V_SCR_HEIGHT];
...
//    logo("  effects");
    for (i = 0; i < 10; ++i) {
        spr[i] = Z_getspr("TFOG", i, 0, sprd + i);
    }
    for (; i < 15; ++i) {
        spr[i] = Z_getspr("IFOG", i - 10, 0, sprd + i);
    }
    ...
    for (i = 0; i < MAXFX; ++i) {
        switch (fx[i].t) {
            ...
            case BUBL:
                ...
                fx[i].y += fx[i].yv;
                //
                if ((b = fld[fx[i].y >> 11][fx[i].x >> 11]) < 5 || b > 7) {
                    fx[i].t = 0;
                }
                break;
        }
    ...
    for (i = 0; i < MAXFX; ++i) {
        ...
        switch (fx[i].t) {
            ...
            case IFOG:
                ...
                break;
            case BUBL:
                // TODO: Verify `c` parameter
                V_dot((fx[i].x >> 8) - w_x + 100, (fx[i].y >> 8) - w_y + HT / 2 + 1 + w_o, 0xC0 + fx[i].s);
                continue;
        }
        if (s >= 0) {
            Z_drawspr(fx[i].x, fx[i].y, spr[s], sprd[s]);
        }
    }
}
...
void FX_bubble(int x, int y, int xv, int yv, int n) {
    ...
    for (; n > 0; --n) {
        ...
        fx[i].t = BUBL;
        fx[i].s = rand() & 3;
        fx[i].x = (x << 8) + d2random(513) - 256;
        fx[i].y = (y << 8) + d2random(513) - 256;
        fx[i].xv = xv;
        fx[i].yv = yv - d2random(256) - 768;
    }
}
